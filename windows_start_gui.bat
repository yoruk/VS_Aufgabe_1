...
start "RMI Server " cmd /c "%JAVA_PATH%/java.exe" -cp %PROJ_PATH% -Djava.rmi.server.codebase=file:///%PROJ_PATH% -Djava.security.policy=%PROJ_PATH%/server.policy rmi.MessageServiceServer & timeout /t 2 /nobreak >nul


:: 3. Client starten (Parameter zum Setzen des Hosts und des Ports: localhost 1099)
start "RMI Client" cmd /k "%JAVA_PATH%/java.exe" -cp %PROJ_PATH% -Djava.rmi.server.codebase=file:///%PROJ_PATH% -Djava.security.policy=%PROJ_PATH%/client.policy rmi.MessageServiceClient
